...
class DEMO:
    ...
    def run_examples(
        ...
        if self.smpl_model is not None:
            ...
            example_output_dir = output_dir + "/smpl_para2proto_pymomentum"
            os.makedirs(example_output_dir, exist_ok=True)
            proto_vertices = (
                self.example_smpl_parameters_to_proto_with_pymomentum_multiple_identity(
        ...
        if self.smplx_model is not None:
            ...
            example_output_dir = (
                output_dir + "/smplx_mesh2proto_pytorch_single_identity"
            )
            os.makedirs(example_output_dir, exist_ok=True)
            proto_parameters = (
                self.example_smplx_meshes_to_proto_with_pytorch_single_identity(
        ...
        if self.smplx_model is not None:
            ...
            example_output_dir = output_dir + "/proto_para2pytorch_single_identity"
            os.makedirs(example_output_dir, exist_ok=True)
            self.example_proto_parameters_to_pytorch_single_identity(
                proto_parameters, example_output_dir
            )

    def example_smpl_parameters_to_proto_with_pymomentum_multiple_identity(
        ...
        proto_model = PROTO.from_files(lod=1, device=self._device)
        converter = Conversion(
            proto_model=proto_model, smpl_model=self.smpl_model, method="pymomentum"
        )
        conversion_results = converter.convert_smpl2proto(
            smpl_vertices=None,
            smpl_parameters=smpl_paras,
            single_identity=False,
            return_proto_meshes=True,
            return_proto_vertices=True,
            return_proto_parameters=False,
        ...
        for i, mesh in enumerate(conversion_results.result_meshes):
            mesh.vertices /= 100.0
            mesh.export(f"{output_dir}/{i:03d}_result_proto.ply")

        return conversion_results.result_vertices

    def example_smplx_meshes_to_proto_with_pytorch_single_identity(
        ...
        proto_model = PROTO.from_files(lod=1, device=self._device)
        converter = Conversion(
            proto_model=proto_model, smpl_model=self.smplx_model, method="pytorch"
        )
        conversion_results = converter.convert_smpl2proto(
    ...
    def example_proto_parameters_to_pytorch_single_identity(
        self, proto_parameters: dict, output_dir: str
    ):
        ...
        proto_meshes, _ = converter._proto_para2mesh(proto_parameters, return_mesh=True)
        for i, mesh in enumerate(proto_meshes):
            mesh.vertices /= 100.0
            mesh.export(f"{output_dir}/{i:03d}_proto.ply")

        conversion_results = converter.convert_proto2smpl(
            proto_vertices=None,
