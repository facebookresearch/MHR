...
"""SMPL(X) to PROTO Model Conversion

This module provides comprehensive tools for converting between SMPL/SMPLX and PROTO body models.

## Workflow Overview

1. **Input Processing**: Accept SMPL(X) / PROTO vertices or parameters as target.
2. **Topology Mapping**: Use precomputed barycentric mappings between mesh topologies to get the
    target PROTO / SMPL(X) vertices.
3. **Parameter Fitting**: Optimize PROTO / SMPL(X) parameters to match target vertex positions.
4. **Output Generation**: Return PROTO / SMPL(X) parameters, meshes, and/or vertices as requested.
...
## Basic Usage
    ...
    # Convert SMPL vertices to PROTO parameters
    result = converter.convert_smpl2proto(
    ...
    # Access results
    proto_params = result.result_parameters
    proto_meshes = result.result_meshes
    ```
"""
...
from fitting_proto_with_pymomentum import PyMomentumModelFitting
...
class FittingMethod(enum.Enum):
    """Enumeration of available fitting methods for PROTO model conversion.
...
class ConversionResult:
    """Data structure containing the results of SMPL to PROTO conversion.

    Attributes:
        result_meshes: List of PROTO mesh objects (trimesh.Trimesh) if requested.
                      None if return_proto_meshes was False.
        result_vertices: Numpy array of PROTO vertices [B, V, 3] if requested.
                        None if return_proto_vertices was False.
        result_parameters: Dictionary containing PROTO model parameters if requested.
                          Contains 'lbs_model_params', 'shape_space_params', and 'face_expr_coeffs' keys.
                          None if return_proto_parameters was False.
    """
...
class Conversion:
    """Main class for converting between SMPL(X) and PROTO model representations.

    This class provides functionality to convert SMPL or SMPLX model parameters
    and vertices to PROTO model parameters using various optimization methods.
    ...
    def __init__(
        ...
        Args:
            proto_model: PROTO body model for conversion target.
        ...
        self._proto_model = proto_model.to(self._DEVICE)
    ...
    def convert_smpl2proto(
        ...
        return_proto_meshes: bool = False,
        return_proto_parameters: bool = False,
        return_proto_vertices: bool = False,
        return_fitting_errors: bool = True,
        batch_size: int = 8,
    ) -> ConversionResult:
        """
        Convert SMPL(X) meshes or model parameters to PROTO model parameters.
        ...
        Args:
            ...
            return_proto_meshes: Whether to return the PROTO meshes. If True, the function will return a list of PROTO meshes.
            return_proto_parameters: Whether to return the PROTO parameters. If True, the function will return a dictionary of PROTO parameters.
            return_proto_vertices: Whether to return the PROTO vertices. If True, the function will return a numpy array of PROTO vertices.

        Returns:
            ConversionResult containing:
                - result_meshes: List of PROTO meshes (if return_proto_meshes=True)
                - result_vertices: Numpy array of PROTO vertices (if return_proto_vertices=True)
                - result_parameters: Dictionary with 'lbs_model_params', 'shape_space_params', and 'face_expr_coeffs'
                (if return_proto_parameters=True)
        ...
        target_vertices = self._compute_target_vertices(smpl_vertices, "smpl2proto")
        if self._method == FittingMethod.PYMOMENTUM:
            fitting_parameter_results = self._s2p_fit_proto_using_pymomentum(
            ...
            fitting_parameter_results = self._s2p_fit_proto_using_pytorch(
        ...
        result_meshes = None
        proto_vertices = None
        if return_proto_meshes or return_proto_vertices:
            result_meshes, proto_vertices = self._proto_para2mesh(
                fitting_parameter_results, return_mesh=return_proto_meshes
            )

        return ConversionResult(
            result_meshes=result_meshes if return_proto_meshes else None,
            result_vertices=proto_vertices if return_proto_vertices else None,
            result_parameters=None
            if not return_proto_parameters
            else fitting_parameter_results,
            result_errors=errors if return_fitting_errors else None,
        )

    def convert_proto2smpl(
        self,
        proto_vertices: torch.Tensor | np.ndarray | None = None,
        proto_parameters: dict[str, torch.Tensor] | None = None,
        single_identity: bool = True,
        is_tracking: bool = False,
        ...
        """
        Convert PROTO meshes or model parameters to SMPL(X) model parameters.

        Args:
            proto_vertices: The vertex positions of the PROTO model. Can be a torch tensor, numpy array or None.
            proto_parameters: The parameters of the PROTO model. Can be a dictionary of torch tensors or None
            single_identity: Whether to use a single identity for all results. If True, a common identity parameters will be optimized
                for all the input PROTO data. If False, each input PROTO data will have its own identity parameters.
            is_tracking: Whether the input PROTO data is a temporal sequence. If True, the function will use the previous frame's parameters as the initial parameters for the next frame.
        ...
        """
        if proto_vertices is not None:
            proto_vertices = self._to_tensor(proto_vertices)
        else:
            if proto_parameters is None:
                raise ValueError(
                    "proto_parameters must be provided if proto_vertices is None."
                )
            _, proto_vertices = self._proto_para2mesh(
                proto_parameters, return_mesh=False
            )
            proto_vertices = self._to_tensor(proto_vertices)

        target_vertices = self._compute_target_vertices(proto_vertices, "proto2smpl")

        # For PROTO to SMPL, we only support pytorch solution
        if self._method == FittingMethod.PYTORCH:
            ...
            raise ValueError(
                "We only support pytorch solution for PROTO -> SMPL conversion!"
            )
    ...
    def _compute_target_vertices(
        self, source_vertices: torch.Tensor, direction: str = "smpl2proto"
    ) -> torch.Tensor:
        """
        ...
        system differences between SMPL (meters) and PROTO (centimeters).
        ...
        Args:
            ...
            direction: Direction of conversion. Must be either "smpl2proto" or "proto2smpl".
        ...
        Raises:
            ValueError: If direction is not "smpl2proto" or "proto2smpl".
        """
        # Load appropriate mapping
        if direction == "smpl2proto":
            mapped_face_id, baryc_coords = self._load_surface_mapping_SMPL2PROTO()
            source_faces = self._smpl_model.faces
        elif direction == "proto2smpl":
            mapped_face_id, baryc_coords = self._load_surface_mapping_PROTO2SMPL()
            source_faces = self._proto_model.character_gpu.mesh.faces.cpu().numpy()
        else:
            raise ValueError(
                f"Invalid direction: {direction}. Must be 'smpl2proto' or 'proto2smpl'"
            )
        ...
        for source_verts in source_vertices:
            ...
            if direction == "proto2smpl":
                # PROTO uses centimeters but SMPL uses meters
    ...
    def _s2p_define_trainable_variables(
        ...
        """Define trainable variables for SMPL to PROTO optimization.
        ...
        # Get pose parameter size from PROTO character
        num_pose_param = int(
            self._proto_model.character.parameter_transform.pose_parameters.sum() - 6
        ...
        num_scale_params = int(
            self._proto_model.character.parameter_transform.scaling_parameters.sum()
        )
        ...
        num_pca_comp = self._proto_model.identity_model.blend_shapes.shape[0]
        ...
        num_face_expr = (
            self._proto_model.face_expressions_model.blend_shapes.shape[0]
            if self._proto_model.face_expressions_model is not None
            else 0
        )
    ...
    def _p2s_define_trainable_variables(
        ...
        """Define trainable variables for PROTO to SMPL optimization.
    ...
    def _s2p_optimize_one_batch(
        ...
        """
        Optimize one batch of PROTO model parameters against target data.
        ...
        """
        self._concat_proto_lbs_model_parameters(variables)
        batched_proto_parameters = self._get_batched_body_model_parameters(
            variables, batch_start, batch_end, self._DEVICE, is_proto_parameters=True
        )
        ...
        proto_verts = self._proto_model(
            identity_coeffs=batched_proto_parameters["identity_coeffs"],
            model_parameters=batched_proto_parameters["lbs_model_params"],
            face_expr_coeffs=batched_proto_parameters["face_expr_coeffs"],
            apply_correctives=True,
        )
        proto_edges = proto_verts[:, edges[:, 1], :] - proto_verts[:, edges[:, 0], :]

        # Compute edge loss using absolute difference
        edge_loss = torch.abs(proto_edges - target_edges).mean()

        # Compute total loss
        if edge_weight is not None:
            vertex_loss = torch.square(proto_verts - target_verts_batch).mean()
    ...
    def _s2p_fit_proto_using_pymomentum(
        self,
        ...
        """
        Fit PROTO model to target vertices using PyMomentum hierarchical optimization.
        ...
        Args:
            target_vertices: Target vertices tensor in PROTO mesh topology with shape [B, V, 3]
                           where B is number of frames, V is PROTO vertices count.
        ...
        Returns:
            Dictionary containing fitted PROTO model parameters:
        ...
        if self.pymomentum_solver is None:
            self.pymomentum_solver = PyMomentumModelFitting(self._proto_model)
        fitting_parameter_results = {
            "lbs_model_params": [],
            "shape_space_params": [],
            "face_expr_coeffs": [],
        }
        # First, per-frame fit of PROTO model to target vertices.
        logger.info("Fitting per-frame PROTO model to SMPL...")
        ...
        if single_identity:
            ...
            num_identity_bs = self._proto_model.identity_model.blend_shapes.shape[0]
            num_expr_bs = self._proto_model.face_expressions_model.blend_shapes.shape[0]
            scale_mask = (
                self._proto_model.character.parameter_transform.scaling_parameters
            )
            ...
            logger.info("Fitting single-identity PROTO model to SMPL data...")
            for i, target_verts in enumerate(tqdm(target_vertices)):
    ...
    def _s2p_fit_proto_using_pytorch(
        self,
        target_vertices: torch.Tensor,
        single_identity: bool,
        batch_size: int = 8,
        ...
        """
        Fit PROTO model to target vertices using PyTorch optimizer.

        Args:
            target_vertices: Target vertices in PROTO mesh topology for each frame
        ...
        Returns:
            Fitted PROTO model parameters [B, num_params]
        """
        # Get proto mesh edges.
        proto_template_mesh = trimesh.Trimesh(
            self._proto_model.character_gpu.mesh.rest_vertices.cpu().numpy(),
            self._proto_model.character_gpu.mesh.faces.cpu().numpy(),
            process=False,
        )
        edges = self._to_tensor(proto_template_mesh.edges_unique).int()
        ...
        for batch_start in tqdm(
            ...
            with torch.no_grad():
                ...
                variables["transls"][batch_start:batch_end] = (
                    target_verts_batch - proto_verts
    ...
    def _p2s_fit_smpl_using_pytorch(
        ...
        edges = (
            torch.from_numpy(smpl_template_mesh.edges_unique).long().to(self._DEVICE)
        )
    ...
    def _load_surface_mapping(self, direction: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        Args:
            direction: Conversion direction ("smpl2proto" or "proto2smpl")
    ...
    @lru_cache
    def _load_surface_mapping_SMPL2PROTO(self) -> tuple[np.ndarray, np.ndarray]:
        """Load SMPL/SMPLX to PROTO surface mapping (cached)."""
        return self._load_surface_mapping("smpl2proto")

    @lru_cache
    def _load_surface_mapping_PROTO2SMPL(self) -> tuple[np.ndarray, np.ndarray]:
        """Load PROTO to SMPL/SMPLX surface mapping (cached)."""
        return self._load_surface_mapping("proto2smpl")

    def _concat_proto_lbs_model_parameters(
    ...
    def _proto_para2mesh(
        self,
        proto_parameters: dict[str, torch.Tensor],
        return_mesh: bool = True,
        batch_size: int = 8,
        verbose: bool = False,
    ) -> tuple[list[trimesh.Trimesh], np.ndarray]:
        """
        Convert PROTO parameters to meshes and vertices.

        Args:
            proto_parameters: Dictionary containing PROTO parameters
        ...
        """
        faces = self._proto_model.character_gpu.mesh.faces.cpu().numpy()
        meshes = []
        proto_vertices = []
        num_samples = proto_parameters["lbs_model_params"].shape[0]

        if verbose:
            logger.info("Converting PROTO parameters to meshes...")
        for batch_start in tqdm(range(0, num_samples, batch_size)):
            batch_end = min(batch_start + batch_size, num_samples)
            batch_parameters = self._get_batched_body_model_parameters(
                proto_parameters, batch_start, batch_end, self._DEVICE
            )
            with torch.no_grad():
                proto_verts = self._proto_model(
                ...
                proto_vertices.append(proto_verts.detach().cpu().numpy())
        proto_vertices = np.concatenate(proto_vertices, axis=0)
        if return_mesh:
            for vertices in proto_vertices:
                meshes.append(trimesh.Trimesh(vertices, faces, process=False))

        return meshes, proto_vertices
    ...
    def _s2p_reprocess_failure_cases(
        ...
        """
        Reprocess failure cases of SMPL->PROTO conversion using PyMomentum approach.
        ...
        Args:
            fitting_parameter_results: Initial PROTO fitting results with 'lbs_model_params',
                                     'shape_space_params', and 'face_expr_coeffs' tensors
            smpl_vertices: SMPL vertices tensor in SMPL mesh topology [B, V_s, 3]
            target_vertices: Target vertices tensor in PROTO mesh topology [B, V_t, 3]
        ...
        for frame_idx in failure_indices:
            ...
            for step in range(interpolation_steps):
                ...
                # Convert SMPL vertices to PROTO space
                proto_interp_vertices = self._compute_target_vertices(
                    smpl_output.vertices.detach(), "smpl2proto"
                )[0]  # [V_proto, 3]

                interpolated_vertices_sequence.append(proto_interp_vertices)
            ...
            # Step 3.3: Run _s2p_fit_proto_using_pymomentum with is_tracking=True
            improved_results = self._s2p_fit_proto_using_pymomentum(
                sequence_vertices, single_identity=False, is_tracking=True
            )

            # Step 3.4: Get the fitting result for the final frame and update the proto result parameters
            final_frame_idx = -1  # Last frame in the sequence

            # Get the error after reprocessing.
            improved_verts = self._proto_model(
            ...
            # Update the proto result parameters if the error is lower than the original error.
    ...
    def _s2p_evaluate_conversion_error(
        ...
        Args:
            fitting_parameter_results: Dictionary containing PROTO fitting results with
                                     'lbs_model_params', 'shape_space_params', and 'face_expr_coeffs'
            target_vertices: Target vertices tensor in PROTO mesh topology [B, V, 3]
            batch_size: Number of frames to process in each batch for memory efficiency
        ...
        for batch_start in range(0, num_frames, batch_size):
            ...
            batch_vertices = self._proto_model(
    ...
    def _get_batched_body_model_parameters(
        ...
        batch_end: int,
        device: torch.device,
        is_proto_parameters: bool = False,
    ) -> dict[str, torch.Tensor]:
        """
        Get SMPL parameters for a batch of frames.

        Args:
            parameter_dict: Dictionary containing PROTO fitting results with
                                    'lbs_model_params', 'shape_space_params', and 'face_expr_coeffs'
        ...
        if (not is_proto_parameters) and self.smpl_model_type == "smplx":
