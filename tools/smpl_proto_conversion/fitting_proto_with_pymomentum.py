"""PyMomentum-based fitting classes for PROTO model conversion

This module provides PyMomentum-based optimization tools for fitting PROTO models
to target vertex positions (assuming we have target positions for each PROTO
...
Example:
    Basic usage for fitting a PROTO model to target vertices:

    >>> fitter = PyMomentumModelFitting(
    ...     proto_model=proto_model,
    ...     num_subsampled_proto_vertices=4000
    ... )
...
class PyMomentumModelFitting:
    """Main class for fitting PROTO models using PyMomentum optimization.

    This class provides a hierarchical optimization approach for fitting PROTO body models
    ...
    Attributes:
        _proto_model: The PROTO body model to be fitted
    ...
    def __init__(
        self,
        proto_model: PROTO,
        num_subsampled_proto_vertices: int = 4000,
    ) -> None:
        """Initialize the PyMomentum model fitting instance.

        Args:
            proto_model: The PROTO body model to fit
            num_subsampled_proto_vertices: Number of vertices to use for efficient fitting.
                                           Reduces computation by clustering similar vertices.
        """
        self._proto_model = proto_model.to(
            "cpu"
        )  # PyMomentum IK solver only works on CPU
        self._num_blendshapes = self._proto_model.identity_model.blend_shapes.shape[0]
        ...
        self._subsampled_vertex_mask: torch.Tensor = self._subsample_proto_vertices(
            num_subsampled_proto_vertices
        )
    ...
    def fit(
        ...
        """Fit the PROTO model to target vertex positions using hierarchical optimization.
    ...
    def _create_body_shape_expression_character(self) -> pym_geo.Character:
        ...
        """
        character = self._proto_model.character

        mean_body_vertices = self._proto_model.identity_model.mean_shape.numpy()
        shapes_basis = self._proto_model.identity_model.blend_shapes.numpy()
        ...
        expression_shapes_basis = (
            self._proto_model.face_expressions_model.blend_shapes.numpy()
        )
    ...
    def _create_hierarchical_masks(
        ...
        """
        lbs_parameter_names = self._proto_model.character.parameter_transform.names
        parameter2joints_mapping = (
            self._proto_model.character.parameter_transform.transform.numpy()
        ...
        v_indx, j_indx = np.where(self._proto_model.character.skin_weights.index)
        skinning_weight_matrix[
            v_indx, self._proto_model.character.skin_weights.index[v_indx, j_indx]
        ] = self._proto_model.character.skin_weights.weight[v_indx, j_indx]
        ...
        lbs_mask = (
            self._proto_model.character.parameter_transform.rigid_parameters.clone()
        )
    ...
    def _subsample_proto_vertices(
        self, num_subsampled_proto_vertices: int
    ) -> torch.Tensor:
        """Subsample PROTO vertices using clustering for computational efficiency.

        Args:
            num_subsampled_proto_vertices: The number of vertices to subsample

        Returns:
            Boolean mask tensor indicating which vertices are selected
        """
        # Get the vertex positions
        vertex_positions = self._proto_model.character.mesh.vertices

        # Sample the vertices
        selected_indices = _clustering_based_sampling(
            vertex_positions, num_subsampled_proto_vertices
        )
    ...
    def _get_solver_options(
        ...
        Returns:
            SolverOptions configured for efficient PROTO model fitting
        """
